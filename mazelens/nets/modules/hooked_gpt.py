...
from mazelens.nets.modules.hooked_gpt_components import (
    LayerNorm,
    LayerNormPre,
    PosEmbed,
    RMSNorm,
...
class HookedGPT(HookedRootModule):
    """
    ...
    def forward(
            self,
            input: Union[str, List[str], Int[torch.Tensor, "batch pos hidden"]],
        ...
        if len(embed.shape) == 2:
            ...
            tokens = embed[None]
        if embed.device.type != self.cfg.device:
            embed = embed.to(devices.get_device_for_block_index(0, self.cfg))
        ...
        if past_kv_cache is None:
            ...
            batch_size, ctx_length, d_model = embed.shape
            (
                cached_batch_size,
                cache_ctx_length,
                num_heads_in_cache,
        ...
        if self.cfg.positional_embedding_type == "standard":
            pos_embed = self.hook_pos_embed(
                self.pos_embed(embed, pos_offset)
            )  # [batch, pos, d_model]
