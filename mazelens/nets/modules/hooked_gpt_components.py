...
class HookedTransformerKeyValueCache:
    ...
    def separate_batch(self, batch_size: int):
        return [
            HookedTransformerKeyValueCache(
                entries=[
                    HookedTransformerKeyValueCacheEntry(
                        past_keys=entry.past_keys[batch_idx: batch_idx + 1],
                        past_values=entry.past_values[batch_idx: batch_idx + 1],
                    )
    ...
    def detach_tensors(self):
        for entry in self.entries:
            entry.past_keys = entry.past_keys.detach()
            entry.past_values = entry.past_values.detach()
        return self
...
class PosEmbed(nn.Module):
    ...
    def forward(
            self, tokens: Int[torch.Tensor, "batch pos d_model"], past_kv_pos_offset: int = 0
